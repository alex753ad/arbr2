"""
Pairs Trading Backtester v17.0
v17.0 Changes:
...
def run_backtest(prices1, prices2, timeframe='4h', entry_z=None, exit_z=None,
                 stop_z=4.0, max_bars=None, min_bars=None, commission_pct=None,
                 slippage_pct=None,
    ...
    # X-001 FIX: inline defaults синхронизированы с config.yaml TRADE-1
    if entry_z is None: entry_z = CFG('strategy', 'entry_z', 2.5)
    if exit_z is None: exit_z = CFG('strategy', 'exit_z', 0.5)
    if max_bars is None: max_bars = CFG('backtester', 'max_bars', 50)
    if min_bars is None: min_bars = CFG('backtester', 'min_bars', 2)
    if commission_pct is None: commission_pct = CFG('strategy', 'commission_pct', 0.10)
    if slippage_pct is None: slippage_pct = CFG('strategy', 'slippage_pct', 0.05)
    if take_profit_pct is None: take_profit_pct = CFG('strategy', 'take_profit_pct', 2.0)
...
with st.sidebar:
    ...
    if not adaptive_entry:
        entry_z = st.slider("Z входа", 1.5, 4.0, 2.5, step=0.1)
    else:
        entry_z = 2.5
    
    exit_z = st.slider("Z выхода", 0.0, 1.5, 0.50, step=0.1)
    ...
    take_profit_pct = st.slider("💰 Take Profit (%)", 0.0, 5.0, 2.0, step=0.1,
                                 help="v22: Фиксировать прибыль при PnL >= X%. 0=отключено. Рекомендуется 2%.")

...
try:
    ...
    def CFG(section, key=None, default=None):
        # N-01 FIX: синхронизировано с config.yaml TRADE-1 + Z_TRAIL
        # Было: entry_z=1.8, exit_z=0.8, take_profit_pct=1.5 — древние значения
        _d = {'strategy': {'entry_z': 2.5, 'exit_z': 0.5, 'commission_pct': 0.10,
              ...
              'min_correlation': 0.20, 'max_hr_threshold': 5.0, 'min_hr_threshold': 0.05,
              'hr_naked_threshold': 0.15, 'max_pvalue': 0.15},
...
def calculate_quality_score(hurst, ou_params, pvalue_adj, stability_score,
    ...
    Компоненты:
      ...
      Stability:      30  — устойчивость во времени (ANALYSIS-v46 [7]: возвращён с 25)
      Hurst (DFA):    20  — подтверждение mean-reversion
      ADF:            10  — независимый тест стационарности (ANALYSIS-v46 [7]: снижен с 15)
    ...
    Параметры pre-filter (dry-run, не блокируют):
      pvalue_raw: float | None — сырой p-value ДО BH-коррекции.
                  Если передан — логируется в bd['prefilter_raw_pvalue']
                  и выставляется bd['prefilter_would_block'] = True/False.
                  Пара НЕ блокируется — только помечается для анализа.
      ubt_passed: bool | None — результат UBT-теста (v44+). None → штраф 0.

    ANALYSIS-v46 [7] CHANGELOG:
      Stability: 25 → 30. Stability — ключевой предиктор SL: 52/67 SL-сделок
        никогда не шли в плюс = нестабильные пары. v45 снизил вес необоснованно.
      ADF:       15 → 10. ADF тест показывает стационарность исторических данных,
        но не предсказывает скорость возврата Z. Завышенный вес инфлировал Q
        для пар с хорошей историей, но медленным mean-reversion (>16ч hold).
    """
    bd = {}

    # ── PRE-FILTER DRY-RUN ────────────────────────────────────────────────────
    # Логируем raw p-value и флаг «была бы заблокирована» — без блокировки.
    # Когда накопим статистику, решим нужно ли включать жёстко.
    PREFILTER_THRESHOLD = 0.10
    if pvalue_raw is not None:
        bd['prefilter_raw_pvalue'] = round(float(pvalue_raw), 6)
        bd['prefilter_would_block'] = bool(pvalue_raw > PREFILTER_THRESHOLD)
    else:
        bd['prefilter_raw_pvalue'] = None
        bd['prefilter_would_block'] = None  # нет данных — неизвестно

    # FDR (25) — v45: непрерывная шкала + полный вес 25 + cap
    bd['fdr'] = min(25, int(max(0, (0.15 - pvalue_adj) / 0.15) * 25))

    # Stability (30) — ANALYSIS-v46 [7]: возвращён с 25 → 30.
    # Stability — ключевой предиктор SL: пары с низкой стабильностью уходят
    # в убыток без единого движения в нужную сторону.
    bd['stability'] = min(30, max(0, int(stability_score * 30)))

    # Hurst (20) — v45: fallback=5 (v44 ставил 10, что инфлировало Q)
    ...
    # ADF (10) — ANALYSIS-v46 [7]: снижен с 15 → 10.
    # ADF тест ≠ предсказание скорости возврата. Завышенный вес инфлировал Q
    # для медленных пар (hold >16ч), не дающих прибыли через trail.
    bd['adf'] = 10 if adf_passed else 0

    # Hedge ratio (15) — v45: оптимум ≤3.5 (из v44), веса из старой формулы
    if hedge_ratio <= 0 or abs(hedge_ratio) > 30:
        bd['hedge_ratio'] = 0
    elif 0.2 <= abs(hedge_ratio) <= 3.5:
        bd['hedge_ratio'] = 15
    elif 0.1 <= abs(hedge_ratio) <= 7.0:
        bd['hedge_ratio'] = 10
    elif 0.05 <= abs(hedge_ratio) <= 15.0:
        bd['hedge_ratio'] = 5
    else:
        bd['hedge_ratio'] = 0
    ...
    # HR uncertainty modifier — v45: градуированный штраф (3 ступени)
    bd['hr_unc_penalty'] = 0
    if hr_std is not None and hedge_ratio > 0:
        unc_ratio = hr_std / hedge_ratio
        if unc_ratio > 0.7:
    ...
    # UBT penalty — v44+: штраф если uBT провален (ubt_passed=False)
    # None → обратная совместимость, штраф 0
    bd['ubt_penalty'] = -5 if ubt_passed is False else 0

    # Итого: prefilter-поля не суммируются (они только для логирования)
    # Компоненты: FDR=25 + Stability=30 + Hurst=20 + ADF=10 + HR=15 = 100 (ANALYSIS-v46)
    score_keys = ['fdr', 'stability', 'hurst', 'adf', 'hedge_ratio',
                  'crossing_penalty', 'data_penalty', 'hr_unc_penalty', 'ubt_penalty']
    total = max(0, min(100, sum(bd[k] for k in score_keys if k in bd)))
    return int(total), bd
...
def sanitize_pair(hedge_ratio, stability_passed, stability_total, zscore,
    ...
    v37 Wave 1.2: HR проверка через is_hr_safe() — единый источник.
    
    Исключения:
      HR вне допустимого диапазона (is_hr_safe)
    ...
    Returns:
        (passed, reason)
    """
    # v37 Wave 1.2: Единая проверка HR
    try:
        from config_loader import is_hr_safe as _is_hr_safe
        hr_ok, hr_reason = _is_hr_safe(hedge_ratio, hr_std=hr_std)
        if not hr_ok:
            return False, hr_reason
    except ImportError:
        # Fallback если config_loader недоступен
        if hedge_ratio <= 0:
            return False, f"HR={hedge_ratio:.4f} ≤ 0"
        if abs(hedge_ratio) < 0.05:
            return False, f"|HR|={abs(hedge_ratio):.6f} < 0.05"
        if abs(hedge_ratio) > 5.0:
            return False, f"|HR|={abs(hedge_ratio):.1f} > 5.0"
...
def assess_entry_readiness(p):
    """
    v37 Wave 1.6: ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ для оценки входа.
    Импортируется в app.py и monitor — НЕ дублировать.
    
    v37 Wave 1.5: Hurst fallback — если DFA дал 0.500 (fallback),
    использовать hurst_ema если доступен, вместо блокировки.
    
    v37 Wave 1.3: min_entry_z из конфига (default 2.5).
    v37 Wave 1.4: Q порог из конфига (default 60 для scanner, 50 для entry).
    
    Mandatory (все 4 должны быть True):
      1. Статус ≥ READY   2. |Z| ≥ Thr   3. Q ≥ 50   4. Dir ≠ NONE
    Optional (бонусные критерии):
      FDR✅, Conf=HIGH, S≥60, ρ≥0.5, Hurst<0.35, Stab≥3/4
    """
    # v37: Read thresholds from config
    _min_hurst = 0.45
    _q_entry_min = 50
    try:
        from config_loader import CFG as _cfg
        _min_hurst = _cfg('strategy', 'min_hurst', 0.45)
        _q_entry_min = 50  # entry minimum stays 50, scanner uses min_quality=60
    except ImportError:
        pass
    ...
    mandatory = [
        ...
        ('Q ≥ {}'.format(_q_entry_min), p.get('quality_score', 0) >= _q_entry_min,
    ...
    # v37 Wave 1.5: Hurst fallback fix
    # Если DFA дал 0.500 (fallback), но есть hurst_ema — использовать его
    ...
    if hurst_is_fallback:
        ...
        if hurst_ema is not None and hurst_ema != 0.5:
            # EMA доступен и не fallback — использовать вместо DFA
            hurst_val = hurst_ema
            hurst_is_fallback = False
            hurst_ok = hurst_val < 0.35
    ...
    if not all_mandatory:
        ...
        # DFA fallback И EMA недоступен — данных недостаточно
        level, label = 'CONDITIONAL', '🟡 СЛАБЫЙ ⚠️H=0.5'
    elif p.get('cusum_break', False):
        level, label = 'CONDITIONAL', '🟡 УСЛОВНО ⚠️CUSUM'
    elif hurst_val >= _min_hurst:
        level, label = 'CONDITIONAL', f'🟡 УСЛОВНО ⚠️H≥{_min_hurst}'
    elif opt_count >= 3:
